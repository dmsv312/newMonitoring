...
class NotifyLeo extends Command
{
...
//    public ?int $offlineExorde = null;
     ...
     * @var string
     */
    protected $signature = 'notifyleo:cron';
...
//        foreach ($nodes as $node) {
//            if (!$node->is_sync) {
//                $this->result = $this->result . 'Exorde ' . $node->location . ' is down - check the node' . "\n";
//                $this->offlineExorde++;
//            }
//        }
//    }
    ...
    public function updateCosmos(string $projectName)
    {
        ...
        try {
            ...
            $this->offlineArchive++;
            $this->result = $this->result . 'Archive Node ' . $archiveNode->name . ' is down - check the node' . "\n";
            $server = Server::where('name', $archiveNode->location)->first();
            $ssh = new SSH2($server->address);
            $ssh->login('root', Crypt::decrypt($server->password));
            $ssh->exec('systemctl restart ' . $projectName);
            return null;
        }
